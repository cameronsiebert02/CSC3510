...
namespace InClass_1_31
{
    internal class DBConnectMysql
    {
        ...
        {
            string q = string.Format("INSERT INTO candy (item, count,cost) VALUES('{0}','{1}','{2}')",
                                item, count, cost);
            if (this.OpenConnection())
            {
                MySqlCommand cmd = new MySqlCommand(q, connection);
        ...
        {
            string q = string.Format("DELETE FROM candy where id='{0}'", id);
            if (this.OpenConnection())
            {
                MySqlCommand cmd = new MySqlCommand(q, connection);
                cmd.ExecuteNonQuery();
        ...
        {
            string q = string.Format("Update candy set item='{0}', " +
